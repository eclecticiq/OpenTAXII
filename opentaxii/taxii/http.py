...
def get_http_headers(version, is_secure):
    ...
    raise ValueError(
        "Unknown combination: version={}, is_secure={}".format(version, is_secure)
    )


def validate_request_headers_post_parse(
    headers, supported_message_bindings, service_bindings, protocol_bindings
):
...
def validate_request_headers(headers, supported_message_bindings):
    ...
    if headers[HTTP_X_TAXII_CONTENT_TYPE] not in supported_message_bindings:
        raise_failure(
            'TAXII Content Type "{}" is not supported'.format(
                headers[HTTP_X_TAXII_CONTENT_TYPE]
            )
        )
...
def validate_response_headers(headers):
    for h in REQUIRED_RESPONSE_HEADERS:
        if h not in headers:
            raise ValueError("Required response header not specified: {}".format(h))
