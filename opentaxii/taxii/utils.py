...
def is_content_supported(supported_bindings, content_binding, version=None):
    ...
    if not hasattr(content_binding, 'binding_id') or version == 10:
        ...
        subtype = (
            content_binding.subtype_ids[0] if content_binding.subtype_ids else None
        )

    matches = [
        (
            (supported.binding == binding_id)
            and (not supported.subtypes or subtype in supported.subtypes)
        )
        ...
        try:
            ...
            if not result.valid:
                ...
                raise BadMessageStatus(
                    'Request was not schema valid: "{}" for content type "{}"'.format(
                        errors, content_type
                    )
                )
